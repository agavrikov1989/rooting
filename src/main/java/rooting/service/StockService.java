...
public class StockService {
    ...
    public Stock getStock(long id) {
        String sql = "SELECT id, latitude, longitude FROM stock WHERE id = ?";
        return jdbcTemplate.query(sql, new StockMapper(), id).stream().findFirst().orElse(null);
    };

    public List<Stock> getStocks() {
        String sql = "SELECT id, latitude, longitude FROM stock";
        return jdbcTemplate.query(sql, new StockMapper());
    };

    public void saveStock(double latitude, double longitude) {
        String sql =
            "INSERT INTO stock (id, latitude, longitude, creation_time) " +
            "VALUES (nextval('stock_seq'), ?, ?, current_timestamp)";
